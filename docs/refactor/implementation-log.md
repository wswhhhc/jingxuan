...
#### 2026-07-11 campaign 只读闭环

- 新增 `/api/v1/batches` 与 `/api/v1/me/tasks`。
- v1 批次查询按当前用户班级过滤，并兼容旧 `classScopes` 中的班级 ID 与字典值两种存储格式。
- 批次时间输出为 `+08:00` ISO-8601，待办状态映射为稳定字符串。
- 单元测试、OpenAPI/Orval 和前端类型检查通过；批次发布与待办状态机写操作待后续迁移。
...
#### 2026-07-11 portfolio 作品详情契约补全

- 扩展 `GET /api/v1/me/works/{id}` 与 `GET /api/v1/showcase/works/{id}` 的详情 DTO，补齐成员、附件、预览媒体、发布状态、评分摘要、互动计数、标签和 `+08:00` 提交时间。
- 详情路径参数改为字符串接收，OpenAPI/Orval 生成的 TypeScript 参数也为 `string`，避免雪花 ID 在浏览器中发生精度丢失。
- 公开展廊只允许“审核通过且已发布”的作品；接口匿名可访问，但会屏蔽提交人 ID、批次以及成员学号、班级、用户 ID。
- 新增详情映射、ID 解析及公开发布约束测试；`21` 个受影响后端单元测试、OpenAPI/Orval 生成和前端类型检查均通过。
