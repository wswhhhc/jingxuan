...
> [!IMPORTANT]
> 本文保留早期缺陷修复历史。文中提到的 `init_schema.sql`、`work_schema.sql` 及手工 `sed` 导入流程已经退役；当前数据库由 `backend/src/main/java/db/migration/V1__Baseline.java`、`backend/src/main/resources/legacy-sql/` 与 `backend/src/main/resources/db/migration/V*.sql` 统一交给 Flyway 管理。
