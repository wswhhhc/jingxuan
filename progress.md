...
## 会话：2026-07-12（阶段 B-C 审计与关键策略调整）

### 审计发现
1. **后端 27 个 v1 Controller 覆盖全部核心业务** — 认证、作品、批次、审核、评分、排行等
2. **前端 8 个学生页面完整** — 路由、布局、页面组件都存在
3. **核心缺口**：
   - OpenAPI YAML 与 Controller 不同步 → Orval 生成客户端不可用
   - 前端页面使用旧 @/api/admin/* 等（未切换生成客户端）
   - CampaignTaskCompletion 接口无实现类（workflow 模块依赖断裂）
   - 旧子代理 9 个测试与生产代码不匹配
   - 后端无法在无 MySQL/Redis 环境下启动（OpenAPI 导出受阻）

### 策略调整
- 不再深挖旧子代理的 9 个测试修复（不阻断业务）
- OpenAPI 同步优先级提升（打通前端的最大瓶颈）
- 新增 sync-openapi.mjs 脚本框架

### 已提交
- 579bf37 — sync-openapi.mjs 脚本框架
- 78cfa58 — logout-all 端点
- 54157c4 — 工具链与 JDK 21 统一
- 49922f — 前端测试基线恢复
- 9948914 — 绿色基线恢复

### 后续建议
1. 实现 sync-openapi.mjs 全功能 → 生成完整 YAML
2. npm run api:generate → 生成 Orval 客户端
3. 逐个页面切换到生成客户端
4. 最终遗留清理 + 生产部署
